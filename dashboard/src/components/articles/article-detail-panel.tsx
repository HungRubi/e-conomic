            ...
            <div className='mt-3'>
              {contentBlocks ? (
                <HomePageEditor
                  ...
                  onChange={full => { setContentBlocks(full.sections); setContentDirty(true); }}
                  disabled={!crud.canUpdate}
                />
              ) : (
                <div className='flex flex-col items-center gap-3 rounded-md border border-dashed border-border/60 py-10 text-center'>
                  <p className='text-sm text-muted-foreground'>
                    Nội dung hiện tại là văn bản thô (Markdown/HTML). Chuyển sang soạn thảo trực quan?
                  </p>
                  <Button
                    size='sm'
                    variant='outline'
      ...
      <div className='mt-3'>
        ...
        {article.coverImageUrl && canUpdate ? (
          <div className='mt-3 flex justify-end'>
            <Button type='button' variant='ghost' size='sm' onClick={() => void setCover(null)} disabled={busy}>
              Gỡ ảnh
            </Button>
          </div>
        ) : null}
