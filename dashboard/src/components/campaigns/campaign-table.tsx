...
function formatDateRange(row: AdminCampaignRow): string {
...
type CampaignTableProps = {
	...
	loading: boolean;
	error: string | null;
	onOpenDetail: (row: AdminCampaignRow) => void;
	onDelete: (row: AdminCampaignRow) => void;
	onRetry: () => void;
	canDelete: boolean;
};
			...
			<Table>
				...
				<TableBody>
					{loading ? (
						...
						<TableErrorStateRow colSpan={7} message={error} onRetry={onRetry} />
					) : rows.length === 0 ? (
