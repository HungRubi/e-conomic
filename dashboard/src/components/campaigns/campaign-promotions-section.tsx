...
export function CampaignPromotionsSection({ campaignId }: CampaignPromotionsSectionProps) {
  ...
  const [formAppliesTo, setFormAppliesTo] = React.useState<'ALL_PRODUCTS' | 'PRODUCTS' | 'CATEGORIES'>('ALL_PRODUCTS');
  ...
  React.useEffect(() => { void load(); }, [campaignId]);
        ...
        {loading ? (
          ...
          <div className='flex flex-col items-center gap-2 rounded-md border border-dashed border-border/60 py-6 text-center'>
            <p className='text-sm text-muted-foreground'>
              Chưa có mã giảm giá nào trong chiến dịch này.
            </p>
          </div>
        ) : (
          <Table>
            ...
            <TableBody>
              {promotions.map(row => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant={row.status === 'ACTIVE' ? 'success' : row.status === 'DRAFT' ? 'secondary' : 'muted'}>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align='end'>
                        <DropdownMenuItem onClick={e => { e.stopPropagation(); void handleRemove(row.id); }}>
