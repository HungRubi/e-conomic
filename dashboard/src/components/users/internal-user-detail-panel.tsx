...
export function InternalUserDetailPanel() {
	const params = useParams<{ userId: string }>();
	const userId = params.userId ?? '';

	const { data, isLoading, error, refetch } = useQuery({
		queryKey: ['admin-internal-user', userId],
	...
	return <DetailContent user={data} onChanged={() => void refetch()} />;
}

function DetailContent({ user, onChanged }: { user: AdminUserRow; onChanged: () => void }) {
				...
				<div className='flex min-w-0 items-start gap-3'>
					...
					<div className='min-w-0'>
						...
						<div className='mt-1 flex items-center gap-2'>
							<h1 className='truncate text-lg font-semibold tracking-tight'>{user.name ?? user.email}</h1>
							<Button
								type='button'
								variant='ghost'
								size='icon'
								className='size-7 text-muted-foreground'
								onClick={() => copyToClipboard(user.id, 'đã sao chép ID')}
						...
						<div className='mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground tabular-nums'>
							...
							<span className='inline-flex items-center gap-1'>
								...
								Tạo {fmtUserDate(user.createdAt)}
							</span>
							{user.updatedAt !== user.createdAt ? (
								<>
									<span aria-hidden>·</span>
									<span>Cập nhật {fmtUserDate(user.updatedAt)}</span>
								</>
							) : null}
						</div>
					</div>
			...
			<div className='grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_400px]'>
				<div className='min-w-0 space-y-4'>
					<section className='dashboard-slide-up rounded-xl bg-card p-4 ring-1 ring-foreground/10'>
						<SectionHeading icon={UserIcon} title='Thông tin tài khoản' />
					...
					<section className='dashboard-slide-up dashboard-stagger-1 rounded-xl bg-card p-4 ring-1 ring-foreground/10'>
						<SectionHeading icon={ShieldIcon} title='Vai trò & quyền hạn' />
						<div className='mt-3 space-y-1'>
							{user.role === 'CUSTOMER' ? (
								<p className='text-sm text-muted-foreground'>
									Đây là tài khoản khách hàng — quản trị từ trang Khách hàng.
								</p>
							) : (
								<EditableField
									label='Vai trò'
									...
									disabled={!crud.canUpdate || isSelf}
									onSave={v => patch({ role: v as AdminUserRow['role'] })}
								/>
							)}
							{isSelf ? (
								<p className='text-xs text-muted-foreground'>
									Không thể đổi vai trò chính mình. Nhờ admin khác đổi nếu cần.
								</p>
							) : null}
						</div>
					</section>
			...
			<AlertDialog open={confirmDelete} onOpenChange={open => !actionBusy && setConfirmDelete(open)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							<span className='font-medium text-foreground'>{user.email}</span> sẽ bị xoá vĩnh viễn.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className='flex flex-col gap-0 p-0 sm:max-w-110'>
				...
				<FieldGroup className='flex-1 overflow-y-auto px-6 py-5'>
					...
					<Field>
						<FieldLabel htmlFor='internal-user-detail-pw'>Mật khẩu mới</FieldLabel>
						<Input
							id='internal-user-detail-pw'
							type='password'
