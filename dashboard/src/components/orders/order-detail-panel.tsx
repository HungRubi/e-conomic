...
function OrderDetailContent({ order, onChanged }: { order: OrderRow; onChanged: () => void }) {
			...
			<div className='grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_400px]'>
				...
				<div className='min-w-0 space-y-4'>
					...
					<section className='dashboard-slide-up dashboard-stagger-1 rounded-xl bg-card p-4 ring-1 ring-foreground/10'>
						...
						<FieldHint>
							Bấm vào từng ô để nhập/sửa thông tin vận chuyển. Nhập mã vận đơn để khách có thể tra cứu đơn
							hàng.
						</FieldHint>
					</section>
					...
					<section className='dashboard-slide-up dashboard-stagger-2 rounded-xl bg-card p-4 ring-1 ring-foreground/10'>
						...
						<FieldHint>
							Bấm vào "Trạng thái thanh toán" để chuyển giữa: Chờ thanh toán &gt; Đã thanh toán &gt; Thất
							bại &gt; Đã hoàn tiền.
						</FieldHint>
					</section>
					...
					<section className='rounded-xl bg-card p-4 ring-1 ring-foreground/10'>
						...
						<div className='mt-3 space-y-3'>
							<div className='flex items-center justify-between gap-2 rounded-md bg-muted/30 px-3 py-2 text-sm'>
								...
								<Badge variant={STATUS_STYLE[order.status]}>{ORDER_STATUS_LABEL[order.status]}</Badge>
							</div>
							<div className='flex items-center justify-between gap-2 rounded-md bg-muted/30 px-3 py-2 text-sm'>
								<span className='text-muted-foreground'>Thanh toán</span>
								<Badge variant='outline'>{PAYMENT_STATUS_LABEL[order.paymentStatus]}</Badge>
							</div>
			...
			<AlertDialog open={shippingOpen} onOpenChange={setShippingOpen}>
				<AlertDialogContent>
						...
						<AlertDialogDescription asChild>
							<div className='space-y-3'>
								<p>
									...
									{' sẽ chuyển sang trạng thái "đã giao vận".'}
								</p>
								...
								<div className='space-y-2'>
									<label className='text-xs font-medium text-foreground'>Đơn vị vận chuyển</label>
									<Input
										value={shipProvider}
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction onClick={() => void handleShippedConfirm()} disabled={shipBusy}>
			...
			<AlertDialog open={refundOpen} onOpenChange={setRefundOpen}>
				<AlertDialogContent>
						...
						<AlertDialogDescription>
							<div className='space-y-3'>
								<p>
									{'Đơn '}
									<strong className='text-foreground'>{order.orderNumber}</strong>
									{' sẽ chuyển sang trạng thái "đã hoàn tiền".'}
								</p>
								{order.paymentMethod === 'COD' ? (
									<div className='rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 space-y-1.5'>
										<p className='flex items-start gap-2'>
											...
											/>
											<span>
												<strong>COD:</strong> Khách chưa thanh toán. Bạn{' '}
												<strong>không cần chuyển tiền</strong>. Đơn vị vận chuyển đã thu hộ và
												sẽ trả lại cho bạn.
											</span>
										</p>
										<p className='flex items-start gap-2'>
											...
											<span>
												Bước này chỉ cập nhật trạng thái trên hệ thống, không ảnh hưởng đến tiền
												bạc.
											</span>
										</p>
									</div>
									...
									<div className='rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 space-y-1.5'>
										<p className='flex items-start gap-2'>
											...
											/>
											<span>
												<strong>BANKING:</strong> Khách đã thanh toán{' '}
												<strong>{formatCurrency(order.totalVnd)}</strong>. Bạn phải{' '}
												<strong>chuyển khoản lại</strong> cho khách trước khi xác nhận.
											</span>
										</p>
										<p className='flex items-start gap-2'>
											<AlertTriangleIcon
											...
											/>
											<span>
												Chỉ bấm xác nhận khi <strong>đã chuyển tiền xong</strong>. Hành động này
												không thể hoàn tác.
											</span>
										</p>
									</div>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction onClick={() => void handleRefund()} disabled={refundBusy}>
