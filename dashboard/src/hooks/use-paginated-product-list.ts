...
import type { AdminProductRow, ListProductsParams, ProductListResponse } from '@/api/admin-products';
import { AuthApiError } from '@/auth/auth-api';

export type ProductListSortKey = NonNullable<ListProductsParams['sortBy']>;

export function usePaginatedProductList(
	listFn: (params: ListProductsParams) => Promise<ProductListResponse>,
	searchInput: string,
	sortBy: ProductListSortKey,
	sortOrder: 'asc' | 'desc',
	pageSize: number,
	status: 'all' | NonNullable<ListProductsParams['status']> = 'all'
) {
	...
	const [rows, setRows] = React.useState<AdminProductRow[]>([]);
	...
	const upsertRow = React.useCallback(
		(row: AdminProductRow, opts?: { prependOnInsert?: boolean }) => {
			let inserted = false;
			setRows(prev => {
