...
export function usePaginatedProductCategoryList(
	listFn: (params: ListProductCategoriesParams) => Promise<ProductCategoryListResponse>,
	searchInput: string,
	sortBy: ProductCategoryListSortKey,
	sortOrder: 'asc' | 'desc',
	pageSize: number,
	status: 'all' | NonNullable<ListProductCategoriesParams['status']> = 'all',
	level: 'all' | 0 | 1 | 2 = 'all'
) {
	const deferredSearch = React.useDeferredValue(searchInput.trim());

	const filterKey = React.useMemo(
		() => `${deferredSearch}\0${sortBy}\0${sortOrder}\0${pageSize}\0${status}\0${level}`,
		[deferredSearch, sortBy, sortOrder, pageSize, status, level]
	);
	...
	const [rows, setRows] = React.useState<AdminProductCategoryRow[]>([]);
	...
	}, [listFn, deferredSearch, sortBy, sortOrder, pageSize, pageForRequest, status, level]);
	...
	const refetch = React.useCallback(
		...
		[listFn, deferredSearch, sortBy, sortOrder, pageSize, page, status, level]
	);

	const upsertRow = React.useCallback(
		(row: AdminProductCategoryRow, opts?: { prependOnInsert?: boolean }) => {
			let inserted = false;
			setRows(prev => {
