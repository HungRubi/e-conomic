...
import type { AdminCampaignRow, CampaignListResponse, ListCampaignsParams } from '@/api/admin-campaigns';
import { AuthApiError } from '@/auth/auth-api';

export type CampaignListSortKey = NonNullable<ListCampaignsParams['sortBy']>;

export function usePaginatedCampaignList(
	listFn: (params: ListCampaignsParams) => Promise<CampaignListResponse>,
	searchInput: string,
	sortBy: CampaignListSortKey,
	sortOrder: 'asc' | 'desc',
	pageSize: number,
	status: 'all' | NonNullable<ListCampaignsParams['status']> = 'all'
) {
	...
	const [rows, setRows] = React.useState<AdminCampaignRow[]>([]);
	...
	const refetch = React.useCallback(
		...
		[listFn, deferredSearch, sortBy, sortOrder, pageSize, page, status]
	);

	const upsertRow = React.useCallback(
		(row: AdminCampaignRow, opts?: { prependOnInsert?: boolean }) => {
			let inserted = false;
			setRows(prev => {
