...
import type {
	AdminCustomerFeedbackRow,
	CustomerFeedbackListResponse,
	ListCustomerFeedbacksParams,
} from '@/api/admin-customer-feedbacks';
import { AuthApiError } from '@/auth/auth-api';

export type CustomerFeedbackListSortKey = NonNullable<ListCustomerFeedbacksParams['sortBy']>;

export function usePaginatedCustomerFeedbackList(
	listFn: (params: ListCustomerFeedbacksParams) => Promise<CustomerFeedbackListResponse>,
	searchInput: string,
	sortBy: CustomerFeedbackListSortKey,
	sortOrder: 'asc' | 'desc',
	pageSize: number,
	status: 'all' | NonNullable<ListCustomerFeedbacksParams['status']> = 'all'
) {
	...
	const [rows, setRows] = React.useState<AdminCustomerFeedbackRow[]>([]);
	...
	const upsertRow = React.useCallback((row: AdminCustomerFeedbackRow) => {
		setRows(prev => {
