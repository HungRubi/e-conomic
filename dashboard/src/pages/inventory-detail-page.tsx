    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-lg'>
        <DialogHeader>
          ...
          <DialogDescription>
            Các nguyên liệu thành phần bị trừ trong lần nhập kho này.
          </DialogDescription>
        </DialogHeader>
        {loading ? (
            ...
            {components.map(c => (
              <div key={c.id} className='flex items-center gap-3 py-2.5'>
                ...
                <div className='text-right text-sm tabular-nums'>
                  ...
                  <p className='text-xs text-muted-foreground'>{c.quantityPerUnit} viên/sp × {c.unitsProduced} sp</p>
                </div>
              </div>
            ))}
          </div>
...
function TransactionRow({
  ...
  let batchItems: { sku: string; name: string; quantity: number; variantId?: string; image?: string | null; label?: string; stockQuantity?: number }[] = [];
      ...
      <TableRow className='cursor-pointer' onClick={() => isBatch && setExpanded(!expanded)}>
        ...
        <TableCell className='h-10 align-middle'>
          <Badge variant={TX_COLOR[tx.changeType] ?? 'outline'} className='text-[10px]'>
            {tx.changeType === 'RECEIVE' && isBatch ? 'Nhập (nhiều biến thể)' : TX_LABEL[tx.changeType] ?? tx.changeType}
          </Badge>
        </TableCell>
        ...
        <TableCell>
          {(tx.components?.length ?? 0) > 0 ? (
            <Button variant='ghost' size='icon' className='size-7' onClick={(e) => { e.stopPropagation(); openComponents(tx); }} title='Xem nguyên liệu đã trừ'>
            ...
            <div className='divide-y divide-border/50 border-t border-border/50'>
              {batchItems.map((item, i) => (
                <div key={item.variantId ?? i} className='flex items-center gap-3 px-8 py-2 text-xs'>
                  ...
                  {item.image ? (
                    <img src={publicAssetUrl(item.image)} alt='' className='size-7 shrink-0 rounded border border-border object-cover' loading='lazy' />
                  ) : (
                    <div className='size-7 shrink-0 rounded bg-muted' />
                  )}
                  <span className='flex-1 font-medium truncate'>{item.label}</span>
                  <span className='tabular-nums text-muted-foreground'>Tồn: {item.stockQuantity}</span>
...
export default function InventoryDetailPage() {
  ...
  const receiveMutation = useMutation({
    mutationFn: async ({ qty, note }: { qty: number; note: string }) => {
      ...
      await adjustProductStock(productId!, { changeType: 'RECEIVE', quantity: qty, note: note.trim() || undefined });
    },
    onSuccess: () => {
    ...
    <div className='space-y-4 dashboard-fade-in'>
        ...
        <div className='flex items-start justify-between gap-4'>
          <div className='min-w-0 flex-1'>
            <div className='flex items-center gap-2'>
              <Button variant='ghost' size='icon' className='size-7 shrink-0' onClick={() => navigate('/inventory')}>
      ...
      <div className='grid grid-cols-2 gap-3 sm:grid-cols-4'>
        ...
        <div className='rounded-xl bg-card p-3 ring-1 ring-foreground/10'>
          ...
          <p className='mt-1 text-2xl font-bold tabular-nums'>
            {transactions.filter(t => t.changeType === 'DEDUCT').reduce((s, t) => s + Math.abs(t.quantity), 0)}
          </p>
        </div>
        <div className='rounded-xl bg-card p-3 ring-1 ring-foreground/10'>
          ...
          <Table className='table-fixed'>
            ...
            <TableBody>
              {transactions.length === 0 ? (
                ...
                transactions.map(tx => <TransactionRow key={tx.id} tx={tx} openComponents={openComponents} />)
              )}
            </TableBody>
          </Table>
        </div>
              ...
              {orderItems.length === 0 ? (
                ...
                orderItems.map(oi => (
                  <TableRow key={oi.id}>
                    ...
                    <TableCell>
                      <Badge variant={STATUS_BADGE[oi.order.status] ?? 'outline'} className='text-[10px]'>
                        {oi.order.status === 'PENDING' ? 'Chờ xử lý'
                          : oi.order.status === 'CONFIRMED' ? 'Đã xác nhận'
                          : oi.order.status === 'PROCESSING' ? 'Đang xử lý'
                          : oi.order.status === 'SHIPPED' ? 'Đã gửi'
                          : oi.order.status === 'DELIVERED' ? 'Đã giao'
                          : oi.order.status === 'CANCELLED' ? 'Đã hủy'
                          : oi.order.status}
                      </Badge>
                    </TableCell>
                    <TableCell className='hidden sm:table-cell text-xs'>
                      <Badge variant={oi.order.paymentStatus === 'PAID' ? 'success' : 'warning'} className='text-[10px]'>
                        {oi.order.paymentStatus === 'PAID' ? 'Đã thanh toán' : 'Chưa thanh toán'}
                      </Badge>
                    </TableCell>
      ...
      {/* Nhập kho dialog */}
      <Dialog open={receiveOpen} onOpenChange={o => { if (!o) setReceiveVariantQtys({}); setReceiveOpen(o); }}>
        <DialogContent className={(() => {
          ...
          <div className='space-y-4 py-4'>
            <p className='text-sm text-muted-foreground'>
              Sản phẩm: <strong>{product.name}</strong><br />
              {(() => {
                const hasVariants = !product.custom && (product.variants?.length ?? 0) > 0;
                if (hasVariants) return <span className='text-xs text-muted-foreground'>(Nhập số lượng cho từng biến thể)</span>;
                return <>Tồn hiện tại: <span className='tabular-nums'>{product.stockQuantity ?? 0}</span>
                {product.custom ? <span className='ml-2 text-xs text-muted-foreground'>(Custom — sẽ trừ nguyên liệu)</span> : null}</>;
              })()}
            </p>
            ...
            {!product.custom && (product.variants?.length ?? 0) > 0 ? (
              ...
              <>
                <div className='space-y-2'>
                  ...
                  <Input type='number' min={1} value={receiveQty} onChange={e => setReceiveQty(e.target.value.replace(/[^0-9]/g, ''))} placeholder='0' disabled={receiveMutation.isPending} />
                </div>
...
{preview.requirements.map((r, i) => (
                        <div key={i} className='flex items-center gap-3 px-3 py-2.5'>
                          ...
                          <div className='min-w-0 flex-1'>
                            ...
                            <p className='text-xs text-muted-foreground'>
                              {(r.amountVnd).toLocaleString('vi-VN')}₫ × {r.quantityPerUnit} viên/sp
                            </p>
                          </div>
                          <div className='text-right text-sm tabular-nums'>
                            <p className={r.materialStock.sufficient ? 'text-destructive' : 'text-destructive font-semibold'}>
                              -{r.totalDeducted}
                            </p>
            ...
            <div className='space-y-2'>
              ...
              <Input value={receiveNote} onChange={e => setReceiveNote(e.target.value)} placeholder='Nhập kho lần 1, bổ sung hàng...' disabled={receiveMutation.isPending} />
            </div>
          </div>
          <DialogFooter>
            <Button variant='outline' onClick={() => setReceiveOpen(false)} disabled={receiveMutation.isPending}>Hủy</Button>
            <Button
        ...
        <DialogContent>
          ...
          <div className='space-y-4 py-4'>
            ...
            <div>
              ...
              <Input type='number' value={adjustQty} onChange={e => setAdjustQty(e.target.value.replace(/^-?[0-9]*$/.test(e.target.value) ? '' : '', e.target.value))} placeholder='0' disabled={adjustMutation.isPending} />
            </div>
          </div>
          <DialogFooter>
            <Button variant='outline' onClick={() => setAdjustOpen(false)} disabled={adjustMutation.isPending}>Hủy</Button>
            <Button onClick={() => adjustMutation.mutate(Number(adjustQty))} disabled={adjustMutation.isPending || !adjustQty}>
