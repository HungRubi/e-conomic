...
export default function InventoryPage() {
	...
	React.useEffect(() => {
		if (!receiveTarget?.custom || !receiveQty) {
			setPreview(null);
			return;
		}
	...
	return (
		<div className='flex min-h-0 flex-1 flex-col gap-4 dashboard-fade-in'>
			...
			<div className='text-muted-foreground flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between'>
				<span>
					Hiển thị {total === 0 ? 0 : page * pageSize + 1}&ndash;{Math.min((page + 1) * pageSize, total)} /{' '}
					{total}
				</span>
			...
			>
				...
				>
						...
						<p className='text-sm text-muted-foreground'>
							...
							{(() => {
								...
								return (
									<>
										Tồn hiện tại:{' '}
										<span className='tabular-nums'>{receiveTarget?.stockQuantity ?? 0}</span>
										{receiveTarget?.custom ? (
					...
					<DialogFooter>
						<Button variant='outline' onClick={() => setReceiveTarget(null)} disabled={receiveBusy}>
							Hủy
						</Button>
						<Button
