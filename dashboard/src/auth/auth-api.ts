...
async function handleResponse<T>(res: Response): Promise<T> {
	if (!res.ok) {
		...
		throw new AuthApiError(body.message || body.error || 'Request failed', res.status, body.code);
	}
	return res.json();
}
...
export async function updateMyProfile(data: { name?: string; email?: string }): Promise<AuthUserResponse> {
...
export async function requestForgotPassword(email: string): Promise<{ token?: string }> {
...
export async function confirmTotpEnrollment(_code: string): Promise<{ backupCodes: string[] }> {
	throw new AuthApiError('TOTP not implemented on server yet', 501);
}

export async function disableTotp(_password: string, _code: string): Promise<void> {
	throw new AuthApiError('TOTP not implemented on server yet', 501);
}

export async function regenerateBackupCodes(_password: string, _code: string): Promise<{ backupCodes: string[] }> {
	throw new AuthApiError('TOTP not implemented on server yet', 501);
}
