  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Link
        href="/login"
        onClick={onLinkClick}
        className="mx-2 flex items-center gap-3 px-3 py-3 text-sm text-text2 hover:text-text hover:bg-surface2 rounded-radius-btn transition-colors"
      >
        <span className="flex items-center justify-center w-7 h-7 rounded-full bg-accent/10 text-accent shrink-0">
      ...
      <div className="mx-2 my-1 rounded-radius-btn bg-gradient-to-r from-red/10 to-orange/10 border border-red/20 p-3">
        ...
        <div className="flex gap-1.5">
          {['08', '45', '32'].map((v, i) => (
            <span key={i} className="w-8 h-6 flex items-center justify-center rounded-md bg-red text-white text-xs font-bold">
              {v}
            </span>
          ))}
      ...
      <div className="space-y-0.5 mt-1">
        ...
        <Link
          href="/"
          onClick={onLinkClick}
          className={`mx-2 flex items-center gap-3 px-3 py-2 rounded-radius-btn text-sm transition-colors ${
            !activeSlug
      ...
      <div className="space-y-1 px-0">
        ...
        <Link href="/orders" onClick={onLinkClick} className="mx-2 flex items-center gap-3 px-3 py-2 rounded-radius-btn text-sm text-text2 hover:text-text hover:bg-surface2 transition-colors">
          <Truck className="w-4 h-4" /> Tra cứu đơn hàng
        </Link>
        <Link href="/?sort=rating" onClick={onLinkClick} className="mx-2 flex items-center gap-3 px-3 py-2 rounded-radius-btn text-sm text-text2 hover:text-text hover:bg-surface2 transition-colors">
          <TrendingUp className="w-4 h-4" /> Bán chạy nhất
        </Link>
        <Link href="/?tag=new" onClick={onLinkClick} className="mx-2 flex items-center gap-3 px-3 py-2 rounded-radius-btn text-sm text-text2 hover:text-text hover:bg-surface2 transition-colors">
      ...
      <div className="space-y-2 px-0 mt-2">
        <Link
          href="/?tag=sale"
          onClick={onLinkClick}
          className="mx-2 flex items-center gap-3 px-3 py-2.5 rounded-radius-btn bg-surface2 hover:bg-border transition-colors group"
        >
