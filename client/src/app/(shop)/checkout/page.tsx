          ...
          {step === 'payment' && (
            <div className="rounded-xl border border-border/80 bg-surface/90 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.06)] sm:p-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block text-sm font-semibold text-text">{method.label}</span>
                        <span className="mt-1 block text-sm leading-6 text-text2">{method.desc}</span>
                      </span>
                      <span className={`mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${selected ? 'border-text bg-text text-bg' : 'border-border'}`}>
              ...
              <div className="mt-5 flex justify-between gap-3">
                <Button variant="ghost" onClick={() => setStep('shipping')} icon={<ChevronLeft className="h-4 w-4" />}>
                  Quay lại
                </Button>
          ...
          {step === 'review' && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between gap-3">
                <Button variant="ghost" onClick={() => setStep('payment')} icon={<ChevronLeft className="h-4 w-4" />}>
                  Quay lại
                </Button>
          ...
          return (
            <div key={step.key} className="min-w-0">
              <div className="flex items-center gap-2">
                <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${active ? 'bg-text text-bg' : 'bg-surface2 text-text2'}`}>
                  {done ? <Check className="h-4 w-4" strokeWidth={2.2} /> : index + 1}
                </span>
                <span className="hidden min-w-0 sm:block">
                  <span className={`block truncate text-sm font-semibold ${active ? 'text-text' : 'text-text2'}`}>{step.label}</span>
