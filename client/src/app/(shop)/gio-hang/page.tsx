	...
	return (
		<label
			className={`relative flex h-5 w-5 shrink-0 cursor-pointer items-center justify-center rounded-md border-2 transition-all duration-200 ${
				checked ? 'border-text bg-text' : 'border-border/60 bg-transparent hover:border-text/60'
			}`}
		>
			<input
				type='checkbox'
...
export default function CartPage() {
	...
	const shipping = selectedSubtotal >= FREE_SHIPPING_THRESHOLD || selectedSubtotal === 0 ? 0 : SHIPPING_FEE;
	const total = selectedSubtotal + shipping;
	const remainingForFreeShipping = Math.max(0, FREE_SHIPPING_THRESHOLD - selectedSubtotal);
	const shippingProgress = Math.min(100, Math.round((selectedSubtotal / FREE_SHIPPING_THRESHOLD) * 100));
	const checkoutHref =
		selectedIds.length > 0 ? `/thanh-toan?items=${encodeURIComponent(selectedIds.join(','))}` : '/thanh-toan';
		...
		<section className='mt-10 border-t border-border pt-8 md:mt-14 md:pt-10'>
			...
			<div className='grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 xl:grid-cols-4'>
				{suggestedProducts.map((product, index) => (
					<ProductCard key={product.feedKey} product={product} index={index % products.length} />
				))}
			</div>
		</section>
	);
