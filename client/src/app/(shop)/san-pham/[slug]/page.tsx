...
import { ChevronLeft, ShoppingBag, Heart, Truck, RotateCcw, ShieldCheck, PackageCheck, Sparkles } from 'lucide-react';
...
export default function ProductDetailPage() {
	...
	const discount = product.compareAtPrice ? Math.round((1 - displayPrice / product.compareAtPrice) * 100) : 0;
...
function ProductGallery({ product, selectedImage, setSelectedImage, discount, reduceMotion }: ProductGalleryProps) {
		...
		>
			<div className='rounded-3xl border border-border/50 bg-surface p-5 shadow-[0_24px_70px_rgba(0,0,0,0.07)] md:p-6 lg:p-7'>
				...
				<div className='mb-4 flex flex-wrap items-center gap-2'>
					...
					<Badge variant={isAvailable ? 'success' : 'error'}>{isAvailable ? 'Còn hàng' : 'Hết hàng'}</Badge>
				</div>
		...
		<section className='mt-12 md:mt-16 max-w-3xl'>
			...
			<div className='lg:hidden grid gap-8'>
				<div className='max-w-xl'>
					<h2 className='text-2xl font-bold tracking-tight text-text md:text-3xl'>Thông tin sản phẩm</h2>
